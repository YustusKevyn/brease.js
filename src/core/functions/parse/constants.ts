...
export const springFunctionPattern = createFunctionPattern(["spring"], [], [numberPattern, numberPattern, numberPattern, numberPattern, numberPattern]);
...
function createFunctionPattern(name: string[], required: string[] = [], optional: string[] = []){
  ...
  // Optional
  let optionalArg = (i: number = 0): string => {
    if(i >= optional.length) return "";
